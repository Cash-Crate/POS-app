...
func ServeHttp() {
	...
	mux.HandleFunc("POST /api/items/create", m.JWTAuthMiddleware(handlers.CreateItem))
	mux.HandleFunc("PUT /api/items/{id}", m.JWTAuthMiddleware(handlers.UpdateItem))
	mux.HandleFunc("PUT /api/items/{id}/{quantity}", m.JWTAuthMiddleware(handlers.SellItem))
	mux.HandleFunc("DELETE /api/items/{id}", m.JWTAuthMiddleware(handlers.DeleteItem))
	mux.HandleFunc("GET /api/items/attrs/{id}", m.JWTAuthMiddleware(handlers.GetItemAttrs))
	mux.HandleFunc("POST /api/items/attrs/create/{id}", m.JWTAuthMiddleware(handlers.CreateItemAttrs))
	mux.HandleFunc("PUT /api/items/attrs/{id}/{name}", m.JWTAuthMiddleware(handlers.UpdateItemAttr))
